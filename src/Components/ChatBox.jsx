...
const quickReplies = {
    ...
    driverToManager: [
        ...
        'Need help with delivery address.',
        'Will be late due to weather.'
    ],
    managerToCustomer: [
...
const ChatBox = ({ roomId, senderId, senderRole, receiverRole, quickReplyType, onClose, receiverName, receiverAvatar }) => {
    ...
    if (roomId === 'driver-manager-ORD101') {
        initialMessages = [
            { id: 1, sender_id: senderId, content: 'Hello Alex, please confirm delivery.', created_at: new Date().toISOString(), delivered: true, read: true },
            { id: 2, sender_id: 'Alex', content: 'Order delivered. Thank you!', created_at: new Date(Date.now() - 60000).toISOString(), delivered: true, read: true },
            { id: 3, sender_id: senderId, content: 'Update on delivery status?', created_at: new Date(Date.now() - 120000).toISOString(), delivered: true, read: false },
        ];
